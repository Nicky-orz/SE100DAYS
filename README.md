...
- 易错点：模拟题要按题意随时修正；构造题里 `need = n(n-1)/2 - m` 小于 n 时直接把图补成完全图，否则取度数最大的点为中心去连所有非邻接点，用 `n-1-maxdeg` 条边把最大通信难度降到 2，连线后要立刻在邻接矩阵置位以防重边，新增边数必须小于 n；超集枚举题的关键是位运算把“图的邻接关系”压缩成了“二进制位的包含关系”，所以可以用Kruskal算法，变成超集枚举题， `i & j = k` 时所有超集都能经 k 连通，按 a 从小到大枚举超集并用并查集合并即可，不必枚举全部点对，但 n=20 时枚举量仍会超时，只能拿部分分
- 状态：已完成(2/3)
...
#### 位运算
- 当一个问题可以被拆解成若干个独立的“是/否”决策时，就能用位运算,Eg:MST选还是不选
